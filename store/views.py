...
def dashboard(request):
    ...
    total_items = (
        Item.objects.all().aggregate(Sum("quantity")).get("quantity_sum", 0.00)
    )
...
class DeliveryListView(
    ...
    #views to display list of deliveries
    model = Delivery
    pagination = 10
...
class CategoryCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy('category_detail', kwargs ={'pk' : self.objects.pk})
