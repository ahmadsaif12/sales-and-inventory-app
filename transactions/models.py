...
class Sale(models.Model):
    ...
    def __str__(self):
        return (
            f"Sale ID : {self.id} " | f"Grand Total : {self.grand_total}" | f"Date :{self.date_added}"
        )

    # functionreturns total quantity of products in the sale
    def sum_products(self):
        return sum(detail.quantity for detail in self.saledetails.all())
...
class SaleDetail(models.Model):
    sale = models.ForeignKey(Sale, on_delete=models.CASCADE, db_column="sale", related_name="saledetails")
    ...
    def __str__(self):
        return (
            f"Detail ID : {self.id}" | f"Sale ID :{self.sale.id}" | f"Quantity : {self.quantity}"  
        )
